...
void EventLoop( mu_Context *ctx );
void DoEvent( EventRecord *event, mu_Context *ctx );
...
char logbuf[64000] = "asdf";
int logbuf_updated = 0;
static float bg[3] = { 90, 95, 100 };


static void log_window(mu_Context *ctx) {
  if (mu_begin_window(ctx, "My Window", mu_rect(10, 10, 240, 186))) {
	  ...
	  mu_textbox(ctx, logbuf, sizeof(logbuf));
...
{	
	...
	mu_init(ctx);
	ctx->text_width = quickdraw_font_get_text_width;
...
void EventLoop(mu_Context *ctx)
{
	...
	do {
        ...
        if (lastMouseHPos != mouse.h || lastMouseVPos != mouse.v) {
        	...
        	mu_input_mousemove(ctx, mouse.h, mouse.v);
        }
        ...
        //if (gotEvent || firstOrMouseMove) {
			...
			mu_end(ctx);
		        ...
		        writeSerialPort(boutRefNum, "quickdraw_render");
		    #endif

        	quickdraw_render(FrontWindow(), ctx);

			#ifdef MAC_APP_DEBUGGING

		    	writeSerialPort(boutRefNum, "quickdraw_render complete");
		    #endif
        //}
...
void DoEvent(EventRecord *event, mu_Context *ctx) {
	...
	switch ( event->what ) {
        ...
        case mouseUp:
            ...
            {
                case inContent:
                    quickdraw_handle_event(event, ctx);
                    break;
                default:
                	break;
            }
